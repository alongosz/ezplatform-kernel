...
final class DoctrineDatabase extends Gateway
{
    ...
    public function archiveEntries(array $actions): void
    {
        $queryBuilder = $this->connection->createQueryBuilder();
        $queryBuilder
            ->update($this->table)
            ->set('is_original', ':is_original')
            ->set('id', ':new_id')
            ->where('action IN (:actions)')
            ->andWhere('is_original = 1')
            ->setParameter('is_original', 0, ParameterType::INTEGER)
            ->setParameter('new_id', $this->getNextId(), ParameterType::INTEGER)
    ...
    private function historize(
        ...
        if (null !== $newLanguageMask) {
            // archive only a part of language mask by setting a new one
            $query->set(
                'lang_mask',
                $query->createPositionalParameter($newLanguageMask, ParameterType::INTEGER)
            );
        }
    ...
    public function loadUrlAliasData(array $urlHashes, ?int $languageMask = null): array
    {
        ...
        if (null !== $languageMask) {
            ...
            // make sure is_alias=0 (system URL) and is_original=1 (active entry) was fetched first if no language was given
            $query
                ->orderBy('u.is_alias', 'ASC')
                ->addOrderBy('u.is_original', 'DESC');
        }
    ...
    public function loadActionToParentMapForAutogeneratedEntries(array $actions): array
    {
        $queryBuilder = $this->connection->createQueryBuilder();
        $queryBuilder
            ->select(['DISTINCT action', 'parent'])
            ->from($this->table, 'url_alias')
            ->where('is_alias = 0')
            ->andWhere('is_original = 1')
            ->andWhere('action IN (:actions)')
            ->setParameter('actions', $actions, Connection::PARAM_STR_ARRAY);

        $rows = $queryBuilder->execute()->fetchAll(FetchMode::ASSOCIATIVE);

        $actionToParentMap = [];
        foreach ($rows as $row) {
            $actionToParentMap[$row['action']] = $row['parent'];
        }

        return $actionToParentMap;
    }

    public function generateSystemEntry(
        string $action,
        string $parentAction,
        string $textHash,
        string $text,
        ...
        $existingEntry = $this->loadAutogeneratedEntryByHashAndParentAction(
            $textHash,
            $parentAction
        );
        if (empty($existingEntry)) {
            $existingEntry = $this->loadAutogeneratedEntry($action);
            $newEntryId = (int)($existingEntry['id'] ?? $this->getNextId());
            // parent for system entries of a given action should be the same
            $parentId = (int)($existingEntry['parent'] ?? $this->loadAutogeneratedEntry($parentAction)['id']);
            $this->insertRow(
                [
                    'action' => $action,
                    'action_type' => 'eznode',
            ...
            $existingEntryLanguageMask = (int)$existingEntry['lang_mask'];
            // skip identical entry
            if ($existingEntryLanguageMask === $languageMask) {
                return;
            }
            // split language mask for original and archived entry
            $commonLanguageMask = $existingEntryLanguageMask & $languageMask;
            // extract part of mask which does not match current one
            $languageMaskForArchivedEntry = \abs($existingEntryLanguageMask - $commonLanguageMask);
            if ($languageMaskForArchivedEntry > 1) {
                // insert archived entry with a new id
                $archivedEntry = $existingEntry;
                unset($archivedEntry['id']);
                $archivedEntry['lang_mask'] = $languageMaskForArchivedEntry;
                $archivedEntry['link'] = $existingEntry['id'];
                $archivedEntry['is_original'] = 0;
                $this->insertRow($archivedEntry);

                $parentId = (int)$existingEntry['parent'];
                $this->updateRow(
                    $parentId,
                    $textHash,
                    $existingEntryLanguageMask,
                    [
                        'lang_mask' => $languageMask,
                    ]
                );
     ...
     * {@inheritdoc}
     *
     * Note: the implementation is similar to the {@see updateRow} method, but has the additional
     * constraint of ID, for the use case during Location swap process, where an old parent
     * for a moment matches rows which shouldn't be updated.
     */
    public function updateParent(
        ...
        $queryBuilder
            ...
            ->andWhere('text_md5 = :text_md5')
            ->andWhere('lang_mask = :language_mask')
            ->andWhere('id = :id')
            ->setParameter('new_parent_id', $newParentId, ParameterType::INTEGER)
            ->setParameter('old_parent_id', $oldParentId, ParameterType::INTEGER)
            ->setParameter('text_md5', $textHash, ParameterType::STRING)
            ->setParameter('language_mask', $languageMask, ParameterType::INTEGER)
            ->setParameter('id', $id, ParameterType::INTEGER);
        $queryBuilder->execute();
    }

    private function loadAutogeneratedEntryByHashAndParentAction(
        string $textHash,
        string $parentAction
    ): array {
        $queryBuilder = $this->connection->createQueryBuilder();
        $queryBuilder
            ...
            ->from($this->table, 'u')
            // @todo parent = 0 ?
            ->join('u', $this->table, 'parent', 'u.parent = parent.id')
            ->where('u.text_md5 = :text_hash')
            ->andWhere('u.is_original = 1')
            ->andWhere('u.is_alias = 0')
            ->andWhere('parent.action = :parent_action')
            ->setParameter('text_hash', $textHash)
            ->setParameter('parent_action', $parentAction)
            ->setMaxResults(1);

        $entry = $queryBuilder->execute()->fetch(FetchMode::ASSOCIATIVE);

        return !empty($entry) ? $entry : [];
    }

    public function archiveActionSystemEntriesNotMatchingHashes(
        string $action,
        array $validHashes
    ): void {
        $queryBuilder = $this->connection->createQueryBuilder();
        $queryBuilder
            ...
            ->set('id', ':new_id')
            ->where('action = :action')
            ->andWhere('is_original = :true')
            ->andWhere('is_alias = :false')
            ->andWhere('text_md5 NOT IN (:valid_hashes)')
            // Legacy DB schema compat: booleans are stored as integers
            ->setParameter('false', 0, ParameterType::INTEGER)
            ->setParameter('true', 1, ParameterType::INTEGER)
            // specification: archived entry gets new ID
            ->setParameter('new_id', $this->getNextId(), ParameterType::INTEGER)
            ->setParameter('action', $action, ParameterType::STRING)
            ->setParameter('valid_hashes', $validHashes, Connection::PARAM_STR_ARRAY);
        $queryBuilder->execute();
    }
}
