...
class Handler implements UrlAliasHandlerInterface
{
    ...
    {
        $location1Swap = new SwappedLocationProperties($location1Id, $location1ParentId);
        $location2Swap = new SwappedLocationProperties($location2Id, $location2ParentId);

        $location1Swap->entries = $this->gateway->loadAllLocationEntries($location1Id);
        $location2Swap->entries = $this->gateway->loadAllLocationEntries($location2Id);

        $location1Swap->mainLanguageId = $this->gateway->getLocationContentMainLanguageId($location1Id);
        $location2Swap->mainLanguageId = $this->gateway->getLocationContentMainLanguageId($location2Id);

        // Load autogenerated entries to find alias ID
        $location1Swap->autogeneratedId = $this->gateway->loadAutogeneratedEntry("eznode:{$location1Id}")['id'];
        $location2Swap->autogeneratedId = $this->gateway->loadAutogeneratedEntry("eznode:{$location2Id}")['id'];
        ...
        $location1Swap->isAlwaysAvailable = $this->maskGenerator->isAlwaysAvailable($contentInfo1['language_mask']);
        $location2Swap->isAlwaysAvailable = $this->maskGenerator->isAlwaysAvailable($contentInfo2['language_mask']);

        $languages = $this->languageHandler->loadAll();

        // Archive everything first to avoid name conflicts in case swapped Locations are siblings
        /*$this->gateway->archiveEntries(
            [
                $this->getSystemUrlAliasAction($location1Id),
                $this->getSystemUrlAliasAction($location2Id),
            ]
        );*/

        /*$o = new MultidimensionalArrayConsoleOutputAdapter();
        $o->display($location2Swap->entries);
        $o->display($location1Swap->entries);*/

        $this->publishUrlAliasesFromRawEntriesForLocation(
            $location1Swap->entries,
            $location2Swap->mainLanguageId,
            $location2Id,
            $location2ParentId
        );

        $this->publishUrlAliasesFromRawEntriesForLocation(
            $location2Swap->entries,
            $location1Swap->mainLanguageId,
            $location1Id,
            $location1ParentId
        );

        $this->internalPublishCustomUrlAliasForLocation($location1Swap, $contentInfo1['language_mask']);
        $this->internalPublishCustomUrlAliasForLocation($location2Swap, $contentInfo2['language_mask']);
    }

    protected function updateSwappedLocationAutogeneratedEntry(array $entries, int $parentId): void
    {
        $autogeneratedEntries = array_filter(
            $entries,
            static function (array $entry): bool {
                return (int)$entry['is_alias'] === 1 && (int)$entry['is_original'] === 1;
            }
        );
        foreach ($autogeneratedEntries as $entry) {
            $this->gateway->updateParent(
                $entry['parent'],
                $entry['text_md5'],
                $entry['lang_mask'],
                $entry['id'],
    ...
    private function publishUrlAliasesFromRawEntriesForLocation(
        array $entries,
        int $mainLanguageId,
        int $locationId,
        ...
        $autogeneratedEntries = array_filter(
            $entries,
            static function (array $entry): bool {
                return (int)$entry['is_alias'] === 0 && (int)$entry['is_original'] === 1;
            }
        );

        $action = $this->getSystemUrlAliasAction($locationId);
        $collectedHashes = array_column($autogeneratedEntries, 'text_md5');
        foreach ($autogeneratedEntries as $entry) {
            $entryLanguageMask = (int)$entry['lang_mask'];
            $isAlwaysAvailable = ($entryLanguageMask & 1) === 1;
            $updatePathIdentificationString = ($entryLanguageMask & $mainLanguageId) > 0;
            $parentAction = $this->getSystemUrlAliasAction($locationParentId);
            $this->gateway->generateSystemEntry(
                $action,
                $parentAction,
                $entry['text_md5'],
                $entry['text'],
        ...
        $this->gateway->archiveActionSystemEntriesNotMatchingHashes(
            $action,
            $collectedHashes
        );
    }
    ...
    private function loadExistingPathEntryData(
        ...
        if (!empty($existingEntry) && $this->isEntryReusable(
                $existingEntry,
                $action,
                $languageMask
            )) {
        ...
        $matchingAliases = \array_filter(
            ...
            static function (array $row) use ($languageMask): bool {
                ...
                // match system URLs which are not archived or archived for exactly the same matching mask
                return $isSystemUrl && (!$isArchived || (int)$row['lang_mask'] === $languageMask);
            }
        );
        ...
        if ($this->isEntryReusable($matchingAlias, $action, $languageMask)) {
    ...
    private function isEntryReusable(array $existingEntry, string $action, int $languageMask): bool
    {
        $isOriginal = (bool)$existingEntry['is_original'];
        $entryLanguageMask = (int)$existingEntry['lang_mask'];
        $isActionReusable = $this->isEntryActionReusable($existingEntry, $action);

        return $isActionReusable && ($isOriginal || $entryLanguageMask === $languageMask);
    }

    private function isEntryActionReusable(array $existingEntry, string $action): bool
    {
        $isCurrentAction = $existingEntry['action'] === $action;
        $isVirtual = $existingEntry['action_type'] === 'nop:';

        return $isCurrentAction || $isVirtual;
    }
}
